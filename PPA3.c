...
// [COPY AND PASTE FROM PPA2] Copy your additional global variables from PPA2 and paste them below
...
// FUNCTION PROTOTYPES
// ================================
// Declare function prototypes here, so that we can organize the function implementations after the main function for code readability.
...
//        to set the stage for the Scavenger Hunt game.
...
void Initialize(void)
{
    ...
    // [COPY AND PASTE FROM PPA2] Copy your initialization routines from PPA2 and paste them below
    ...
    // [TODO] Allocate heap memory for on-demand variables as required.  Initialize them as required.
...
void GetInput(void)
{
    ...
    // [COPY AND PASTE FROM PPA2] Copy your input collection routines from PPA2 and paste them below
...
void RunLogic(void)
{
    // [COPY AND PASTE FROM PPA2] Copy your main logic routines from PPA2 and paste them below

    // [TODO]   Implement the Object Collision logic here
    //
    //      Simple Collision Algorithm
    //      1. Go through all items on the board and check their (x,y) coordinates against the player object x and y.
    ...
    //         Collected String (Mystery String)
    ...
    //      1. Check if the contents of the Collected String exactly match that of the Goal String.
...
void DrawScreen(void)
{
    ...
    // [COPY AND PASTE FROM PPA2] Copy your draw logic routines from PPA2 and paste them below
...
void LoopDelay(void)
{
    MacUILib_Delay(100000); // 0.1s delay, change as needed.
}
...
void GenerateItems(struct objPos list[], const int listSize, const struct objPos *playerPos, const int xRange, const int yRange, const char* str)
{
    ...
    // In order to make sure this candidate is valid, it needs to meet both criteria below:
    ...
    // Thus, for every generated item candidate, check whether its x-y coordinate and symbol have been previously generated.  
    ...
    // There are many efficient ways to do develop this function.
    //  We will take a deep dive into some methods in COMPENG 2SI3.
